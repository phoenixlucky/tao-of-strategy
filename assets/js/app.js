...
async function init() {
  try {
    ...
    // 每日一面：从原始顺序选取，不受洗牌影响
    const _d = new Date();
    const _s = _d.getFullYear() + '-' + (_d.getMonth()+1) + '-' + _d.getDate();
    let _hash = 0;
    for (let i = 0; i < _s.length; i++) { _hash = ((_hash << 5) - _hash) + _s.charCodeAt(i); _hash |= 0; }
    dailyQuote = allQuotes[Math.abs(_hash) % allQuotes.length];
    // 随机排列
...
function showDailyQuote() {
  ...
  if (el) el.innerHTML = buildCard(dailyQuote, true);
}
